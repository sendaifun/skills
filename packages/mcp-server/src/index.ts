 ...
 */
function filterSkills(skills: SkillMetadata[]): SkillMetadata[] {
  if (!selectedSkills) {
    return skills;
  }
...
server.setRequestHandler(GetPromptRequestSchema, async (request) => {
  ...
  try {
    ...
    throw new Error(
      `Skill "${skillName}" not found: ${error instanceof Error ? error.message : "Unknown error"}`
    );
  }
});
  ...
  return {
    tools: [
      ...
      },
      {
        name: "get_skill",
        description:
          "Get the full content of a specific SendAI skill for Solana development",
        inputSchema: {
          type: "object" as const,
          properties: {
            name: {
              type: "string",
              description:
                "The skill name (e.g., kamino, raydium, helius, drift-protocol)",
            },
          },
          required: ["name"],
        },
      },
      {
        ...
        description:
          "Get a specific resource file from a SendAI skill (examples, templates, docs)",
        inputSchema: {
          type: "object" as const,
          properties: {
            skill: {
            ...
            resource: {
              type: "string",
              description:
                "The resource path (e.g., resources/api-reference.md, examples/basic.ts)",
            },
          },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case "get_skill_resource": {
      const { skill, resource } = args as { skill: string; resource: string };
