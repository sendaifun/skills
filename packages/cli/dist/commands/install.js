...
import { downloadSkill, fetchSkillsList, skillExists, } from "../utils/github.js";
import { getClaudeSkillsDir, getSkillInstallPath, normalizeSkillName, } from "../utils/paths.js";
export async function installCommand(skills, options) {
    try {
        ...
        if (options.all) {
            ...
            if (skills.length === 0) {
                ...
                console.log(chalk.gray("Usage: sendai-skills install <skill-name> [skill-name...]"));
        ...
        for (const skillName of skillsToInstall) {
            ...
            try {
                ...
                console.log(chalk.yellow(`  ${skillName}: Already installed (use update to refresh)`));
                skipped++;
                continue;
            }
            catch {
            ...
            try {
                ...
                spinner.fail(`${skillName}: ${error instanceof Error ? error.message : "Installation failed"}`);
                failed++;
            }
        }
        // Summary
        ...
        if (installed > 0) {
            ...
            console.log(chalk.gray("Restart Claude Code or start a new session to use the installed skills."));
        }
    }
    catch (error) {
